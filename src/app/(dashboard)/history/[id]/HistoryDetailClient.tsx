...
type AnyResult = VerdictAnalysisResult | (VerdictAnalysisResult & OldFormatResult) | OldFormatResult;

function isNewFormat(result: AnyResult): result is VerdictAnalysisResult {
  return "whatMessageAbout" in result && typeof (result as VerdictAnalysisResult).whatMessageAbout === "string";
}

function OldFormatDisplay({ result }: { result: OldFormatResult }) {
  return (
    <>
  ...
  return (
    <div className="space-y-6">
      ...
      {isNewFormat(result) ? (
        ...
        <OldFormatDisplay result={result as OldFormatResult} />
      )}
    </div>
  );
}
